...
\cline{2 - 3}
& 10 & Ein oder mehrere Teilnehmer können als Protokllant markiert werden \\
\cline{2 - 3}
...
\begin{description}
		...
		\item Finde heraus, wie die gesetzliche Betreuerin von Frank Heinz heißt und wo Sie wohnt.
		\item Lege ein neues Projekt für  an, 
		\item das die Förderung seiner Selbstständikeit zum Ziel hat.
		\item Übertrage diese Ziele in ein entsprechendes Feld der Betreuungsplanung von Herman Hinz.
		\item Lege ein Protokoll für ein Gespräch mit Herman Hinz über sein Verhalten am Mittagstisch an.
		\item Füge Marlene Friedrich und Klaus Müller als Teinnehmer hinzu.
...
\begin{description}
        ...
        \item[Phase 2] In dieser Phase wurde die Stellen offengelegt, an denen den Usern das Bedinkonzept nicht deutlich wurde. Els Ergebnis wurde zum Beispiel beim \textit{TextTransfer} ein Hilfebutton eingefögt, der eine kleines PopUp mit kurzer Hilfestellung anzeigt. Häufige Problemstellungen waren auch falsche oder uneinheitliche Benennungen von Menüpunkten und Inhalt. Diese Fehler waren sehr dankbar, da leicht zu beheben und verdeutlichen noch einmal den Stellenwert von Usability Tests. Das Konzept, dass \EBP immer einen aktiven Bewohner und eine aktive Wohngruppe hat, war die größte Hürde für die User. Die Informationen über die aktuell aktive Wohngruppe und den aktiven Bewohner werden in einem Informationswidget im oberen Bereich der Anwendung visualisiert (siehe dazu Kapitel \ref{sec:beschreibung}). Dort können die aktiven Elemente auch geändert werden. Bei jedem Test wurde das Bedienkonzept erst klar, als der User das Infowidget endeckte und seine Funktionen  ausprobierte. In allen Fällen war das Konzept nach kurzem Experementieren klar. Eine Verbesserung der Bedienbarkeit sollte jedoch an diesem Widget ansetzten. Die vollständigen Ergebnisse finden sich im Anhang in Form von Protokollen oder in Form von Screencasts mit Tonaufnahmen der Testsitzungen.
