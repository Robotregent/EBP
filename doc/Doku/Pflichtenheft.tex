...
zentralen Server ausgeführt werden und von einem, oder mehreren externen Clients gleichzeitig bedienbar sein. Zusätzlich soll der Zugriff über eine Remotedesktopumgebung erfolgen. Dies soll eine außreichende Reaktionszeit erzielen, wenn der Zugriff von verschiedenen Standorten aus erfolgt, die nur über ein leistungsschwaches Netzwerk verbunden sind. Weiterhin unterbindet dieses vorgehen den Zugriff durch andere MySql Clients und versucht damit den gesetzlichen Verpflichtungen (Siehe \pageref{sec:dasi}) nachzukommen.
...
\subsection{Datensicherheit} \label{sec:dasi}
